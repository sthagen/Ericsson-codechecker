...
pylint --rcfile="$WORKDIR"/.pylintrc -f json --output $JSON_DIR/pylint-reports-clang.json -j0 --py-version=3.11 \
    "$WORKDIR"/codechecker_merge_clang_extdef_mappings \
    "$WORKDIR"/tests/**

WORKDIR="$ROOT"/analyzer/tools/statistics_collector
pylint --rcfile="$WORKDIR"/.pylintrc -f json --output $JSON_DIR/pylint-reports-statictics-collector.json -j0 --py-version=3.11 \
    "$WORKDIR"/codechecker_statistics_collector \
    "$WORKDIR"/tests/**


WORKDIR="$ROOT"/web
pylint --rcfile="$WORKDIR"/../.pylintrc -f json --output $JSON_DIR/pylint-reports-web.json -j0 --py-version=3.11 \
...
WORKDIR="$ROOT"
pylint --rcfile="$WORKDIR"/.pylintrc -f json --output $JSON_DIR/pylint-reports-root.json -j0 --py-version=3.11 \
    "$WORKDIR"/bin/** \
